...
Consider $\abs{S}=2$. We want to find the best probabilities $P^A=p_1^A,p_2^A$ for \A, given that \B plays \crbs. The expected utility are shown in \Cref{tables:exmple's utilities}
...
In \Cref{tables: example result}, we can see \A's probabilities, the best strategy for \B given those values and the respected utility, and finally \A utility given \B strategy.
...
A strong stackleberg equilibrium to this problem will be setting probabilities $P=\{p_1,...,p_k\}$ over $\mathcal{S}$ (a mixed strategy) based on the assumption \B will observe this strategy and choose the optimal response, which in turn \A will try to minimize. \A's task is to find $P_{\A}^*$ as described in \Cref{equations: optimal Pa}; 
...
% Those strategies are shown in \Cref{figures: react to multiple strategies example}.
...
\end{theorem}
\begin{proof}
The follower's utilities are shown in \Cref{tables: follower's utilities- two strategies- with variables}.  Denote the probability that the follower chooses $S_1$ with $p$. Then, one can compute the follower's expected utility for each of its strategies:
\begin{itemize}
    \item $\itf(S_1)$
        \begin{equation*}
...
\begin{enumerate}
    ...
    \item $0 \leq p \leq 1$
\end{enumerate}
The solution to such equations system is shown \Cref{equations: two strategies results}:
\begin{equation}
\begin{aligned}
        c_1 &\in \begin{cases} 
...
\Cref{equations: two strategies results} set conditions over the utilities $c_1,c_2$ and probability $p$, so the follower would prefer $comb(S_1,S_2)$ over either $\itf(S_1)$ or $\itf(S_1)$.
...
% \begin{Example}
% Consider strategies $S_1,S_2,S_3$, showing in \Cref{figures: react to multiple strategies example}. $S_1$ covers the world 'vertically', $S_2$ covers the world 'horizontally', and $S_3$ tries to react to both strategies.
% While simulating, we ran those strategies on $100 \times 100$ world. The results are as follows: 
% \begin{itemize}
%     \item $S_1-S_3$, Leader's reward is 4985, Follower's reward is 5015
%     \item $S_2-S_3$, Leader's reward is 3619, Follower's reward is 6381
% \end{itemize}

% Playing the optimal response to each strategy yield reward of 10000, and playing the optimal response of one strategy against another yield result of ~5000 (random like).

% One can compute the expected reward, depending on the probabilities the follower put on the strategies, and see that for any probability distribution where $p(S_1) < 0.23$, the Follower's expected reward is greater than the Leader's. Therefore, in some cases, the follower best response is to respond to \emph{both} strategies, instead of responding optimally (\itf) to only one.

% \begin{figure}[thpb]
%     \centering
%     \begin{tikzpicture}
% \begin{axis}[
%         title={Strategies $S_1,S_2, S_3$, starting at $(1,1)$},
%         ymin=0, ymax=11,
%         xmin=0, xmax=11,
%         legend style={font=\small,at={(0.5,-0.1)},
% 		anchor=north},]
%     \addplot[blue,
...
% \end{axis}
% \end{tikzpicture}
%     \caption{Strategies layout, one above the other.}
%     \label{figures: react to multiple strategies example}
% \end{figure}
% \end{Example}

% The example above proves that (sometimes,) responding to all the strategies are better than responding optimally to only one. We would like to formalize this problem, and find how to respond to multiple strategies.
